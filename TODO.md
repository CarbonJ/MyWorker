...
### Security
- [x] JIRA URL XSS — validate protocol before rendering `<a href>` (`ProjectDetail.tsx`)
- [x] Import JSON validation — full type/range checks before any destructive SQL (`importExport.ts`)
- [x] Content Security Policy — added `<meta http-equiv="Content-Security-Policy">` to `index.html`
